...
class RuleBase(object):
	...
	def activation_weight_calculation(self):
		...
		for rule_index, rule in enumerate(self.rule_row_list):
			rule_matching_degree = 1
			for attribute_index, attribute in enumerate(self.obj_list):
				rule_matching_degree *= pow(rule.attributes_individual_matching[attribute_index],
											attribute.attribute_weight / max_attribute_weight)
			rule.matching_degree = rule_matching_degree * rule.rule_weight
			# rule.matching_degree = rule_matching_degree
			sum_matching_degree += rule_matching_degree
	...
	def rules_aggregation_analytical_evidential_reasoning(self):
		...
		for j in range(len(product_b)):
			final_consequence[j] = (product_b[j] - product_a) / (
					sum_product_b - product_c - (len(self.con_ref_values)) - 1)
	...
	def dynamic_rule_activation(self):
		...
		factoring_consistency = self.hypertuple_consistency_measure(self.rule_row_list)
		best_performing_factor_consistency = factoring_consistency
		best_rule_set = list()
		count = 0


		while power_factor > 0 and (hyper_tuple_size <= 1 or factoring_consistency <= best_performing_factor_consistency):
			temp_rule_list = list()
			weights = self.weight_calculation(power_factor)
			for weight_index, weight in enumerate(weights):
				if weight > 0:
					temp_rule_list.append(self.rule_row_list[weight_index])
			temp_factoring_consistency = self.hypertuple_consistency_measure()
			if len(temp_rule_list) == 0:
				power_factor = power_factor-1
			elif temp_factoring_consistency > best_performing_factor_consistency:
				best_performing_power_factor = power_factor
				best_performing_factor_consistency = temp_factoring_consistency
				best_rule_set=temp_rule_list
			power_factor = power_factor+1
			count+=1
			# hyper_tuple_size = len(self.rule_row_list)
	...
	def hypertuple_consistency_measure(self, rule_list):
		consequence_rating_count=[0 for _ in range(len(self.con_ref_values))]
		...
		for rule_index, rule in enumerate(self.rule_row_list):
			...
			consequence_rating_count[max_rating] = consequence_rating_count[max_rating] + 1

		overall_max_count = 0
		for index, count in enumerate(consequence_rating_count):
